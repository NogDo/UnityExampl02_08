...
namespace MyProject
{
    public class CAsyncByTaskFixed : MonoBehaviour
    {
        // async : 함수 앞에 붙는 키워드로, 해당 함수가 대기 가능(비동기) 함수라는 의미가 됨.
        // await : async(비동기) 함수 내에서 사용되며, 함수 내의 대기 가능 요소(Task 등)이 완료될 때까지 대기하도록 함.
        // 멀티 쓰레딩을 수행할 때 처럼 새로운 쓰레드를 생성하는 것이 아닌, Task를 생성하여 수행하므로 단일 쓰레드에서 수행이 가능함.
        ...
        async void Start()
        {
            ...
            await Task.WhenAll(breadTask, pattyTask, pickleTask, lettuceTask);
            //.ContinueWith(result => { });
